...
│   │   ├── chunkedExtract.ts       # 타일 40개 초과: 10조각 청크 압축 전사(반분할·인용 후보) + 종합
│   │   ├── extractRounds.ts        # 추출 차수 관리 순수 로직 (요청 1개 = 최대 5청크, 누적 병합·진행 표기)
│   │   ├── sourceTiles.ts          # 원본 → 타일 준비 (/analyze·/extract 공용, 결정적 계산)
│   │   ├── clientFlow.ts           # analyze → extract 반복 → synthesize 클라이언트 루프 (요청 함수 주입)
│   │   ├── routeErrors.ts          # 소스 노트 라우트 공통 오류 응답
│   │   ├── claudeCall.ts           # Sonnet 호출·usage 기록·JSON 파싱·API 오류 원인별 메시지
...
| 2026-09-24 | [PDF 노트 3/3 후속] 추출도 차수별 요청으로 분리 | 추출 단계 300초 위험: 한 요청 안에서 청크 5개를 넘으면 병렬 차수가 늘어 54조각(6청크) ≈ 260초, 150조각(17청크) ≈ 390초 이상. **요청 하나 = 병렬 1차수(최대 5청크)**로 분리 — `/analyze`(첫 요청, 한도 1회 차감)가 1차수를 추출해 phase `extracting`(남음)/`extracted`(끝)로 청크 단위 누적 캐시(`extracted` + `chunkTotal`), 신규 `/extract`가 남은 청크 중 다음 5개만 처리해 누적(phase `extracting`일 때만 AI → 추가 차감 없음), 끝나면 `/synthesize`. 이어 받기는 원본에서 타일을 다시 만든다(가공물 저장 금지 원칙 유지, 결정적 계산 — meta.tileCount·청크 수와 다르면 409). 차수 중 일부가 실패해도 성공분은 먼저 저장 → 재시도 시 `/analyze` 캐시 히트(차감 없음)로 남은 청크만. 모달은 `clientFlow.runSourceNoteFlow`(요청 함수 주입 루프, 진행 없음·요청 12회 초과 시 중단, 다른 분석 시작 시 중단)로 자동 연속 호출, 진행 문구 `추출 k/n → 종합`. 신규 `extractRounds.ts`(nextRoundIndexes·mergeExtracts·extractionProgress·advanceExtraction·toPhaseResponse), `sourceTiles.ts`(타일 준비 공용), `routeErrors.ts`, `chunkedExtract.extractChunkAt`(청크 1개: 반분할·재시도). 검증(AI 호출 없음): 단위 14건 + 가짜 서버(실제 advanceExtraction·toPhaseResponse·runSourceNoteFlow, 메모리 캐시) 흐름 19건 — 12청크 요청 순서 analyze→extract→extract→synthesize·진행 `추출 2/3→3/3→종합`·차감 1회·청크당 1회 추출 / 2차수 중 청크 실패 → 성공분 캐시·재시도는 남은 [7,10,11]만·차감 그대로 1회 / 정확히 5청크는 analyze→synthesize / 무한루프 방지 / 150조각 = 추출 요청 4개, 요청당 최장 175초(가정: 준비 40+청크 135) vs 미분리 580초. 실제 이소정 PDF로 타일 2회 생성 → 43조각 라벨·바이트 해시 동일(결정성, 준비 약 18초). `.claude/settings.local.json`에 `worktree.bgIsolation: none`(커밋 대상 아님) | 100% |
| 2026-09-24 | [PDF 노트 3/3 후속] 분석 2요청 분리 + 개념 새 위키 + 인용 강조·위치 | `6d60fc3`: ①`/analyze`는 분할 경로에서 추출만 하고 phase `extracted`로 캐시, ②`/synthesize`(신규)가 캐시된 extracted만 종합 — 모달이 ① 완료 후 ② 자동 호출, 진행 문구 2/2단계. 한도 차감은 ①에서 1회(②는 extracted일 때만 AI). 종합 출력 상한 섹션당 불릿 6·불릿당 2문장(프롬프트+서버). 개념 이름을 위키용 명사로 정리(괄호 제거·가운뎃점 쌍은 앞쪽·붙여쓰기) + 기존 어휘에 없으면 새 위키 후보 승격(총 3, 개념 출신 우선, 서버 승격분 fromConcept=모달 기본 해제). 인용 후보에 하이라이트·굵은 글씨·인용 박스·도입부 강조 명시 + 형식 지시를 추출 마지막 user 턴에도 반복. 위치 밀림 원인 = 추출문 라벨이 소제목 단위뿐이고 회수 로직이 직전 소제목 라벨을 물려받음 → 인용마다 자기 조각 라벨(범위 금지), 같은 줄 라벨 우선. **실측(이소정 PDF, 추출 1회+종합 1회 $0.793)**: ① 추출 5청크 최장 132초(클라이언트 관측 약 2분), ② 종합 AI 109초·요청 약 122초 — 각각 300초 한도 안. 인용 후보 44개(이전 0), 기억할 문장 5개 전부 원문 글자·위치 일치(p.2 카르마 문장 포함, 이전 p.2로 밀리던 문장은 p.3), 새 위키 3(매출곱셈공식·ANSVA구조·선행지표)+기존 4, 섹션 최대 불릿 4. **사고**: 이전 워크트리를 `git worktree remove --force`로 지울 때 node_modules junction을 따라 메인 node_modules 약 170개 패키지 삭제 → dev 서버 종료. npm install로 복원(빈 껍데기 4개 `@aws-sdk/client-s3`·`sharp`·`import-in-the-middle`·`require-in-the-middle`는 삭제 후 재설치, `npm ls` 정상, lock 무변경). 이후 이 작업은 워크트리 미사용 | 100% |
| 2026-09-24 | [PDF 노트 3/3 후속] 이소정 캡처 PDF 실측 + 수정 4건 | 실제 jsPDF 캡처 PDF(`월부 콘텐츠 기획방법.pdf`, 20쪽·23.4MB, 텍스트 0자)로 실측. **수정** ①(`430ba02`) jsPDF는 20쪽이 이미지 20장이 든 리소스 사전 하나를 공유하고 쪽마다 1장만 `/Ik Do`로 그림 → 리소스 전체의 최대 이미지를 고르던 로직이 전 쪽을 '여러 이미지 조합'으로 오판해 폴백. 콘텐츠 스트림을 풀어 실제 그리는 XObject만 봄 → 20쪽 JPEG 원본 추출, 본문 쪽 크롭 596–1324(1쪽 헤더 0–1324, 19·20쪽 배너 336–1584·327–1920 — 폭 ≤1568이라 해상도 손실 없음), 43조각 ②(`fda1d6b`) 15조각 청크 압축 전사가 6000토큰에 잘림 → 10조각·병렬 6·8000토큰·청크당 1,200단어 + 잘리면 반분할 재추출 ③(`6b8a022`) 실측 중 크레딧 부족이 "AI 분석에 실패했어요"로만 보임 → `describeAnthropicError` 원인별 메시지(크레딧 402·키·과부하·크기), 분할 추출 API 오류를 감싸지 않음 ④(`8b2a394`) 모델이 `===인용 후보===` 구획을 5구간 모두 빼먹고 본문 `> "…"` 블록으로만 인용 → 구획 없으면 인용 블록을 후보로 회수(직전 위치 라벨 부착), 저장된 추출문에도 적용해 [다시 분석] 시 종합만 재호출. **결과**: 분석 268초($0.721: 추출 5구간 최장 119초 + 종합 127초) + 종합 재호출 157초($0.192) = $0.913. 위키 8·태그 4 전부 기존 표기 재사용(새 0, 이웃은 로컬 OPENAI 키 없음). 인용 5개 중 4개 원문 글자 일치, 1개는 끝 '다' 누락(추출 전사 오류), 위치 3/5 정확·2개는 한 쪽 앞으로 표기. **남은 이슈**: 종합 출력이 6~8천 토큰이라 전체가 Vercel 300초 한도에 근접(268초) — 종합 출력 축소 또는 스트리밍 필요, 인용 후보를 본문 회수에 의존하면 위치가 거칠고 전사 오류가 그대로 들어감(구획 준수율 개선 필요), 본문 '내용 정리'의 따옴표 문장은 원문 보장 대상이 아님. 테스트 노트 `7cf4d5a2` 사용자 확인 후 삭제 예정 | 95% |
| 2026-09-24 | [PDF 노트 3/3 후속] 분할 추출 인용 원문 보존 | 분할 추출을 압축 전사로 바꾸면서 종합 단계가 압축 본문에서 "기억할 문장"을 뽑으면 원문이 아니게 되는 문제. 추출 프롬프트에 `===인용 후보===`(QUOTE_MARKER) 구획 — 따옴표·강조·결론 문장을 **압축하지 않고 원문 그대로** `- "문장" (위치)`로 따로 받는다(구간당 ≤10). `ChunkExtract.quoteCandidates`로 저장(재분석 시 재사용). 종합 입력 끝에 `[Q1] "…"` 번호 목록을 붙이고 quotes는 `{id:"Q3"}`로만 고르게 한 뒤, 서버(`resolveChunkQuotes`)가 번호를 **후보 원문+위치로 치환** — 모델이 문장을 고쳐 써도 저장되는 건 원문, 후보에 없는 문장(지어낸 인용)은 버림(공백·따옴표·문장부호 차이만 동일 취급). 청크 경계 겹침으로 같은 후보가 두 번 나오면 1개로. 비분할 경로(이미지 ≤40조각·PDF)는 모델이 원본을 직접 보므로 기존 방식 유지. 검증: 파싱·중복 제거·번호/텍스트 치환·지어낸 문장 제거 9케이스 통과, tsc·ESLint·next build 통과 | 100% |
| 2026-09-24 | [PDF 노트 3/3] 소스 → 요약 노트 자동 생성 | **소스 1세트(PDF 1개 또는 이미지 1~20장) → 요약 노트 1개**. 개념 노트는 만들지 않는다(`[[개념]]` 허브가 곧 개념 노드). 파이프라인 하나 + 입력 어댑터 2개: ①텍스트형 PDF(unpdf 평균 30자/쪽 이상) → Claude document URL(실패 시 1회 base64 ≤31MB) ②이미지형 PDF(jsPDF 웹 캡처·스캔) → pdf-lib로 페이지마다 가장 큰 이미지 XObject가 DCTDecode면 스트림 바이트 = JPEG 원본 추출 → 이미지 경로, 실패(Flate/JBIG2/다중 이미지/Form 중첩)면 URL 폴백 + `image-pdf-fallback` 로그 ③이미지 → **여백 자동 크롭**(행 4px 샘플·양끝 16px 중앙값 배경·차이>20·열 비율>1.5%·패딩 24·85%↓/320px↑일 때만, 세트 안 ±10%면 합집합 통일, 세로 크롭 없음) → 가로>1568 축소 → 타일 높이 min(1568, 1.15MP/가로)·겹침 80px → JPEG q85(요청 25MB 초과 시 q70). 크롭·축소한 결과를 raw로 1회 디코딩 후 잘라 O(n²) 회피. **가공물은 저장하지 않는다**(R2 원본·uploaded_files 무변경). 한도: PDF 100쪽 / 세트 타일 150(서버·클라 같은 `computeTiles`·같은 원본 치수로 판정) / **타일 40 초과 → 30개씩(경계 1타일 공유) 병렬 3 전사 → 텍스트 종합**, 청크 1회 재시도 후 실패면 전체 에러(부분 요약 금지), 전사문은 `analysis.extracted`에 저장해 [다시 분석] 시 종합만 재호출. `source_analyses`(0018) 캐시 — 히트는 AI·한도 차감 없음, `ai-source-note` 20회/일. 컨텍스트: 전체 메모(삭제·**잠금 제외**) 위키 300/태그 150을 대표 표기+횟수로. **서버 후처리**: 문법 기호 제거→`resolveToCanonical`→있으면 existing 교정, 새 위키 공백 제거·3개 상한, 태그 `[\w가-힣]`만, 키 중복 제거, 요약 임베딩→`match_memos`(0.4) 상위 5개(잠금 제외)의 위키가 2개↑ 메모에 있거나 개념과 겹치면 `neighbor`(기본 해제). 생성(`/create`)은 AI 재호출 없음 — `buildNoteDoc`이 Tiptap JSON 직접 생성(원본 정보 인용·한 줄 요약·핵심 요약·내용 정리·핵심 개념(확정 위키만 `[[ ]]`)·기억할 문장·연결된 메모·`연결: [[..]]`·`#태그`), **wiki_links/tags는 에디터와 같은 정규식으로 본문에서 추출해 저장**(첫 편집 후 불변) + AI 문장 속 우발적 `#단어`/`[[`는 `＃`/`[ [`로 무력화(원치 않는 태그·허브 방지) → memo_sources(position) → uploaded_files.memo_id 호환 채움 → 임베딩 즉시. UI: MemoList `파일로 노트` 버튼 + 파일 드롭 오버레이(`Files` 타입일 때만 — 메모 이동 드롭과 분리), `SourceNoteModal`(헤더만 읽는 치수 파싱·파일명 자연 정렬·드래그 재정렬·중복(같은 노트·같은 순서)·분석 중 닫아도 유지→토스트 [확인하기]·칩 기존/새로/비슷한 메모·길게/우클릭 이름 수정·자동완성 재사용·폴더·연결된 메모 토글·[다시 분석] 한도 안내), 모바일 vaul 시트. presign `linkedMemos`에 position 추가(순서만 바꾼 세트는 새 분석). SourceFileBar: 세로 2만px↑ 이미지는 탭=다운로드 + 안내 문구. 로그는 `removeConsole` 때문에 `console.error` 채널. 검증: computeTiles 7케이스 기대값 일치(20/34/62/74), 크롭(데스크톱 597–1214·배너 페이지 배너 무시·모바일/사진/카드 미크롭), PDF 분기(합성 jsPDF→이미지형 6쪽 12타일 / Flate 스캔→폴백 / 세제개편안→텍스트형), buildNoteDoc·정규화·헤더 파싱 18케이스 통과, verify-changes.sh·ESLint 클린·`next build` 통과. **E2E(로컬 dev, 실제 Anthropic 호출)**: 사진 1장 low 경고 $0.041 / 카드뉴스 6장 $0.060(수정 후) / 긴 캡처 1080×16000 13조각 $0.113·최하단까지 반영 / 1:18+1:31 묶음 44조각 분할 4청크 $0.313·67초 / 합성 jsPDF 캡처 PDF 6쪽 image-pdf 12조각 $0.099 / 텍스트 PDF 30쪽 $0.244 / Flate 스캔 PDF 폴백 $0.058 / 표 많은 PDF 20쪽 $0.209(수치 정확). 위키 제안 49개 중 기존 재사용 31(63%), 새 위키 ≤3, 띄어쓴 위키 0. **생성 노트 본문 1글자 수정 후 저장 → wiki_links·tags 순서까지 불변(DB 확인)**, 그래프에서 기존 허브 연결, 다운로드 SHA-256 원본 일치, 같은 세트 재업로드 duplicate·순서만 바꾼 세트 새 분석·101쪽/조각 160개 업로드 전 차단·캐시 히트(3초, AI 0회)·분석 중 닫기→완료 토스트 [확인하기]. **E2E 수정 5건**: ①1080×1080 카드가 1.15MP를 살짝 넘어 장마다 2조각 → 타일 높이 15% 이내 초과는 살짝 축소해 1장(카드뉴스 비용 30%↓) ②분할 추출 30조각·충실 전사가 빽빽한 캡처에서 출력 한도에 잘려 실패 → 15조각·병렬 5·**압축 전사**(요점·예시·수치 모두, 문장만 짧게)·잘림은 재시도 안 함(결정적이라 비용만 2배), 실패 시에도 usage 콘솔 기록 ③인용 위치에서 조각 번호를 이미지 번호로 오인("이미지 3") → 라벨 그대로 "이미지 1 · 조각 5" ④뜻이 다른 기존 태그 억지 재사용(`토스`) 금지 문구 ⑤완료 토스트가 사라진 뒤 버튼으로 다시 열면 결과·오류가 초기화 → 파일 없이 열면 유지 + 검토 화면 "다른 파일로". **로컬 전용 주의**: `.env.local`에 `OPENAI_API_KEY`가 없어 로컬에선 생성 노트 임베딩·이웃 추천이 fail-open(빈 결과) — 이웃 로직은 기존 메모 저장 임베딩을 주입해 실데이터 검증(유사 5·잠금 0·`마케팅` neighbor). SW가 dev 청크를 cache-first로 잡아 클라이언트 수정이 안 보이는 현상 2회 재현(서버 CSS엔 있음) — SW 해제+캐시 삭제로 해결. 미검증: 실제 이소정 캡처 PDF(경로 미지정), 모바일 vaul 시트(창 크기 조절 불가). 테스트 데이터(노트 3·파일 14·R2 24객체·분석 9) 정리 완료. 총 AI 비용 ≈$2.1(저장된 9건 $1.22 + 실패한 첫 분할 시도 추정 ≤$0.9) | 99% |
...
| 2026-08-09 | 플래너 추가 개선 2건 (주뷰 레인 라벨·기간 패널) | ① **주뷰 '종일' 라벨 세로 중앙 정렬** — 기존 paddingTop 밀어내기(범위 바 줄 수에 비례)로 라벨이 레인 바닥까지 밀렸음 → flex items-center로 레인 전체 높이 기준 중앙. 헤더 좌측 스페이서 두 칸은 캘린더 관례대로 공백 유지 ② **주/월 뷰 우측 패널 기간 전체 플랜 모드** — 기존엔 뷰와 무관하게 선택일 하루치만 표시(주/월에서 '일요일/1일 플랜'에 그침) → PlanPanel에 periodStart/End/Label props 추가: 범위 플랜은 '기간 플랜' 섹션에 1회만(날짜 그룹마다 반복 방지), 단일일·반복 인스턴스는 날짜별 그룹(종일 우선+시작시간순, 선택일 그룹 보라 강조), 스크롤. 일뷰는 기존 하루치 유지. CalendarView가 주(currentWeek~+6)/월(startOfMonth~endOfMonth) 기간·라벨 전달. 미사용 DeleteMode 타입 제거(ESLint 경고) 검증: tsc 0·ESLint 클린·verify-changes.sh 통과·렌더 하니스(PlanPanel period 모드 340px, 주뷰 1400px) 스크린샷 확인 | 100% |
| 2026-08-09 | 플래너 UX 개선 5건 (뷰 정비 후속) | ① **DayView '오늘' 판정 UTC 버그** — `toISOString()`(UTC) 기반이라 KST 오전 9시 전엔 현재시각 선·오늘 강조가 어제로 판정 → `format(new Date(),'yyyy-MM-dd')` 로컬 기준 통일 ② **범위 바 주 경계 이어짐 신호** — `computeRangeBars`에 continuesLeft/Right 추가, 이어지는 쪽 끝단 flat + 시작 마커(3px 보더)는 실제 시작 주에서만(월뷰 RangeBar·주뷰 레인 공통, 기존 startCol/endCol 판정은 일요일 시작·토요일 종료를 이어짐으로 오판+인라인 borderLeft가 클래스 무력화) + 주뷰 바에 실제 전체 기간 라벨(M/d – M/d, 3칸 이상일 때) + title에 기간 포함 ③ **뷰 모드 전환 시 기간 동기화** — `changeViewMode`: 선택일(없으면 현재 기간 기준점)이 새 뷰에서도 보이도록 currentMonth/currentWeek 정렬(기존엔 10월 보다 주로 바꾸면 마지막 보던 주가 뜸). 일뷰 전환은 기존 selectedDate||today 유지 ④ **주뷰 종일 레인 탭 생성** — 빈 셀 클릭 시 해당 날짜 종일 플랜 폼(initialTime 없으면 폼 종일 기본 on, 칩/바는 stopPropagation이라 충돌 없음) ⑤ **주뷰 초기 스크롤 현재 시각 부근**(일뷰 패턴) + **Google 동기화 버튼 title에 마지막 성공 시각**(localStorage `weave:planner-last-sync`). 검증: tsc 0·ESLint 클린·verify-changes.sh 통과·렌더 하니스 스크린샷 재확인(이어짐 flat/기간 라벨/스크롤 위치) | 100% |
...
| 2026-09-08 | 다크모드 글자색 가독성 + 흰색 팔레트 | 사용자 신고(메모 `4a2133d6`, "외부 복붙 텍스트가 야간모드에서 어둡다") 라이브 진단 후 **원인 2개**를 각각 수정. ① **prose 변수 다크 미적용(신고 메모의 실제 원인)** — globals.css의 `.prose { --tw-prose-*: 라이트값 }` 블록이 `@import "tailwindcss"` **뒤의 unlayered CSS**라 typography 플러그인이 layer 안에서 만드는 `dark:prose-invert`보다 우선. 그 결과 다크모드에서도 `--tw-prose-quotes: #1E293B`가 남아 **blockquote 텍스트가 rgb(30,41,59)로 렌더**(프로덕션 DOM 실측). 신고 메모는 인용구가 본문의 핵심이라 "복붙 텍스트가 어둡다"로 보였음(해당 메모엔 인라인 color mark가 0개 — DB 확인). 링크·코드·불릿·hr·표 테두리도 같이 라이트값이었음. 수정: `.dark .prose`로 같은 변수 재정의(specificity 0,2,0). ② **붙여넣은 인라인 near-black** — 앱 규칙이 "사용자 글자색은 존중"(2026-07-05)이라 `rgb(48,48,56)`(노션류, 41개 메모·65 mark)·`rgb(55,53,47)`·`#000000` 등이 다크모드에서 그대로 렌더돼 안 보였음. `lib/tiptap/adaptiveTextColor.ts` 신규 — `@tiptap/extension-color`를 `.extend()`해 renderHTML만 감싸고, WCAG 상대휘도로 배경 대비를 계산해 **읽을 수 없는 색에만** `data-color-tone`(dark/light)+보정색(`--tc-adapt`)+원본(`--tc`)을 함께 출력. globals.css가 테마가 맞을 때만 보정색으로 교체. 임계값은 실측 대비로 고정 — 다크: 붙여넣기 `rgb(6,95,212)` 3.06 < **3.4** < 팔레트 최저 `#64748B` 3.75 / 라이트: `#FFFFFF` 1.00 < **1.5** < 팔레트 최저 `#EAB308` 1.92 → **팔레트 10색은 전부 원본 유지**. 무채색은 테마 기본색(#F1F5F9/#1E293B), 유채색은 hue·채도 유지한 채 명도만 이동(`rgb(6,95,212)`→`#74aefb`). 형광펜 위 글자는 `--tc`로 원본 복원(배경이 밝으므로 보정이 역효과). **content JSONB 무수정** — 렌더 시점 보정이라 기존 메모 즉시 적용, 마이그레이션 불필요 ③ **흰색 팔레트 추가**(요청) — EditorToolbar `TEXT_COLORS`에 #FFFFFF 추가(9색, grid-cols-4→5), 스와치 테두리 gray-300/600으로 한 단계 진하게(흰·검정이 드롭다운 배경에 묻힘 방지), EditorBubbleMenu `QUICK_COLORS`에도 #FFFFFF. ①②에 의해 흰색/검정은 **테마 자동 대비 쌍**으로 동작. 검증: verify-changes.sh(null 0·파일끝 정상·tsc 0)+ESLint 클린(기존 경고 3건만)+로컬 dev 라이브 대조 — 다크: blockquote #1E293B→#CBD5E1, 붙여넣기 near-black→rgb(241,245,249), 흰글자 유지, 형광펜 위 검정 유지 / 라이트: 붙여넣기 색 원본 그대로, 흰글자→#1E293B, prose 변수 회귀 없음 + 색상 피커 5×2 그리드 양 테마 스크린샷. **주의: 로컬 검증 중 SW 캐시(memo-planner-v5)가 옛 CSS를 서빙해 "수정 미반영"으로 보였음 — 2026-08-02 교훈 재확인.** 미커밋·미배포 (`npm run build`는 다른 세션 dev 서버가 `.next` 점유 중이라 미실행 — push 전 필요) | 100% |
...
| 2026-07-17 | AI UX 개선 1차 (17파일) | AI 기능 UX 전수 점검 후 1차 개선 — **채팅**: ①서버 에러(429 한도/400 길이) 문구를 클라이언트가 그대로 노출(기존엔 일괄 "다시 시도" 오안내) ②스트리밍 Stop 버튼(AbortController, 부분 응답 유지) ③경량 마크다운 렌더러 `ui/Markdown.tsx` 신규(의존성 0, bold/제목/목록/코드) ④추천 칩 즉시 전송 ⑤4,000자 카운터 ⑥`api/ai/chat`에 이번 주 플랜(월요일 시작)+최근 플랜 컨텍스트 추가(기존엔 플랜 데이터 전무 — "플랜 달성률" 추천 질문에 답 불가) ⑦**RAG**: 질문 embedText→match_memos 상위 4개 메모 본문 500자 주입(캐시 블록과 분리로 prompt cache 히트 유지, fail-open) — **인사이트 정확성**: ⑧갭 분석 마운트 자동 로드를 `cache_only=1`로 분리(기존엔 캐시 미스 시 탭 진입만으로 AI 호출·한도 차감) ⑨회고 리포트 에러 구분(모든 실패가 "데이터 없음"으로 표시되던 것) + JSON 파싱 실패 시 24h 캐시 저장 금지(빈 리포트 하루 고착 방지) + cache_only/force 지원 + 기간 선택 시 캐시 자동 표시 ⑩프로필 재분석 cached 응답 시 "완료" 대신 "24h 이내 결과, N시간 후 가능" 토스트 ⑪관심사 분석을 갭 분석과 동일 패턴 통일(cache_only 자동 로드+force 재분석+캐시 배지) + 두 탭에 분석 범위 문구("최근 메모 20개 기준") — **프라이버시**: ⑫chat/insights/report/analyze-profile 4개 라우트 잠금 메모 제외(`is_locked=false`, 그래프 분석과 규칙 통일) + analyze-profile 메모 상한 300개 — **검색/기타**: ⑬`api/memos/search-semantic` 신규 + useMemoSearch FTS 0건 시 시맨틱 폴백(전체 검색만, 폴더/휴지통 제외) + MemoList 안내 배지 ⑭RelatedMemosPanel 임베딩 대기 폴링(2.5s×3, stale memoId 가드)으로 "분석 안 됐어요" 오탐 해소 + 수동 생성 후 즉시 조회 ⑮`api/ai/usage` 신규(서비스 롤로 api_usage 조회) + 인사이트 탭 헤더에 "오늘 사용량 N/M회"(80% 이상 앰버) — 검증: 클라우드 tsc --noEmit 에러 0 + 변경 파일 ESLint 클린(react-hooks 컴파일러 오탐 3건은 프로젝트 선례대로 사유 명시 disable), null byte 0, 파일 끝 정상. 다음 묶음(중기): 메모→플랜 추출, 에디터 슬래시 AI, 주간 회고 푸시, 퀵캡처 자동 분류, 구조화 출력 전환, 그래프 링크 임베딩화, 피드백 루프. **프로덕션 라이브 검증 통과(2026-07-17 배포 후)**: ①인사이트 헤더 사용량 표시(대화 0/300·분석 1/40·생성 0/40, 탭별 버킷 전환) ②추천 칩 즉시 전송→실제 플랜 데이터로 달성률 67%(3중 2) 답변+미완료 항목 지목+RAG로 24.06.19 메모 본문 인용 ③스트리밍 중 Stop 버튼 표시·마크다운(제목/굵게/불릿/구분선) 정상 렌더 ④갭 분석 캐시 자동 표시+캐시됨 배지+범위 문구 ⑤관심사/회고 탭 마운트 시 `cache_only=1` 요청만 발생(네트워크 확인, AI 호출 0)→분석하기로 정상 생성 ⑥검색 '몸을 단련해야 하는 이유'(FTS 0건)→search-semantic 폴백 200+보라 안내 배지+의미 유사 결과 표시 ⑦관련 메모 패널 즉시 로드(유사도 표시, 오탐 없음) ⑧콘솔 에러 0건. 참고: 사용량 카운터는 탭 전환 시점에 갱신(전송 직후 실시간 아님 — 의도된 동작) | 100% |
| 2026-07-31 | 이미지 후속: placeholder UX + give-up 본문 정리 | 네이버 이미지 수정 때 발견한 구조 이슈 2건 처리 — ①ResizableImageView에 깨진 이미지 placeholder: blob 미발견(src=''+localBlobId, 업로드 미완 원본 유실)·로드 실패(onError) 시 브라우저 기본 깨짐 아이콘 대신 원인별 안내 문구 + '다시 시도'(loadError) + 편집 시 '본문에서 제거'(deleteNode). src 변경 시 에러 리셋(업로드 완료 swap 대응) ②`imageSwap.stripImageNodesInContent` 신규(순수 함수, 단위 테스트: 중첩/최상위 제거·no-match 원본 유지·입력 불변) + flushQueue가 give-up된 localBlobId를 수집해 큐 안 memo-insert/body-update 본문에서 깨진 노드를 서버 저장 전 제거 — blob·큐 삭제 후 본문에 영구 깨진 노드가 남던 경로 차단 ③SyncBootstrap: 이미지 give-up 사용자 토스트(기존엔 console.warn만). 한계: 이미 서버에 저장된 깨진 노드는 자동 정리하지 않음 — placeholder의 '본문에서 제거'로 수동 정리(자동 일괄 정리는 데이터 삭제라 의도적으로 보수적 접근). 검증: tsc 0에러+ESLint 클린+strip 단위 테스트 통과 | 100% |
| 2026-07-31 | 네이버 가져오기 메모 이미지 깨짐 수정 | 사용자 신고(모바일 캡처: 깨진 아이콘+alt)를 프로덕션 라이브 진단으로 원인 확정 — 해당 메모 img src가 phinf.pstatic.net(네이버 CDN) 핫링크였고, `withImgCacheVersion`(2026-07-11 PWA 캐시 우회용 ?cv=)이 **모든** 이미지에 무조건 부착되어 네이버 CDN이 예상 밖 쿼리를 거부한 것. 실측: cv 부착 FAIL/원본 URL 정상(1080px)/no-referrer 무관. 오프라인 큐(localBlobId) 가설은 기각. 저장 데이터 무손상(cv는 렌더 시 부착) → utils.ts에서 hostname .r2.dev일 때만 cv 부착하도록 한정(케이스 7종 검증, MemoCard·ResizableImageView 공통 헬퍼라 동시 해결). 참고: 커스텀 도메인 이전 시 판별에 추가 필요. 별도 발견(후속 후보): 업로드 큐 give-up 시 본문의 image node를 정리하지 않아 src=''+localBlobId 노드가 영구 잔존할 수 있는 구조 + 깨진 이미지 placeholder UX 부재. 커밋은 아래 참조 | 100% |
| 2026-07-30 | 관심사 분석 행동 연결·변화 추적 + 슬래시 제목 줄 분리 | ①관심사 분석 '읽고 끝나는 화면' 개선 — 키워드 칩 클릭 시 `/memo?q=키워드` 이동(MemoList가 URL ?q= 초기 검색어 지원, sessionStorage 복원보다 우선. FTS 미일치 시 기존 시맨틱 폴백이 자동 발동) ②변화 추적 — insights 라우트가 type=interest 생성 시 직전(20시간 이전) retro_reports 스냅샷과 키워드 비교(공백 제거·소문자 정규화, count는 실행별 표현 변동이 커서 등장/사라짐만, 각 8개 상한)해 changes{added,removed,prevAt} 저장, BubbleChart '지난 분석 대비 변화' 섹션(새로 등장 emerald/뜸해진 gray, 칩 클릭 시 관련 메모 검색) ③/제목1~3 슬래시 명령도 applyHeadingBlock(삭제→isolateSelectedLines→setNode)으로 현재 줄만 제목 적용(툴바와 규칙 통일). 검증: tsc 0에러+ESLint 클린(기존 경고 3건만). **주의(세션 노하우)**: device_stage_files로 같은 경로를 재스테이징하면 mount 캐시가 이전 사본을 줄 수 있음 — 고유 경로로 복사 후 스테이징하면 우회됨. 커밋 d872401, acc9342 | 100% |
| 2026-07-30 | 플래너 시간·에디터 제목 버그 수정 (7파일) | ①TimePicker 45/15분이 '00'으로 표시되던 버그 — 분 파싱의 10분 단위 반올림(45→50)이 옵션 목록[00,15,30,45]에 없는 값을 만들어 fallback(options[0]) 표시가 원인. 반올림 제거+입력창이 실제 값 표시 ②시/분 필드를 직접 타이핑 가능한 콤보(입력+드롭다운 quick pick, ↑/↓ 증감, blur/Enter 정규화)로 개편 ③종료 시간 자정 지원 — allowMidnight prop으로 24시 옵션(분 00 고정), PlanFormModal·QuickCaptureModal 적용. dragHelpers/뷰/검증은 이미 1440분 지원이라 픽커만 개방. Google Calendar 동기화는 24:00→익일 00:00 변환 ④에디터 H1~H3가 드래그한 줄 주변까지 적용되던 버그 — Enter 1회=hardBreak라 여러 줄이 한 문단이고 toggleHeading이 블록 전체를 바꾸는 구조가 원인. `lib/tiptap/isolateSelectedLines.ts` 신규: 적용 전 선택 줄 앞뒤 hardBreak 제거+split으로 해당 줄만 독립 문단화 후 선택 복원(실제 prosemirror-model/state 단위 테스트 6케이스 통과, EditorToolbar.applyHeading에 연결. SlashCommand의 setNode heading 경로는 미적용 — 후속 후보) ⑤dev HEAD 빌드 깨짐 수정 — 직전 이미지 커밋에서 EditorToolbar에 필수 onImageUpload prop 추가 후 MemoEditor 호출부 미갱신으로 tsc 실패하던 것을 handleImageUpload 연결로 해결. 검증: tsc 0에러+변경파일 ESLint 클린(콤보 role=combobox+aria-controls 정리). 커밋 08a8208, 2a5e85a. **후속 핫픽스**: 라이브 검증에서 to가 split 지점의 기본 매핑(assoc=1)으로 다음 문단 시작에 붙어 다음 줄까지 H1 적용되는 경계 버그 발견 → from assoc 1/to assoc -1 매핑으로 수정, 단위 테스트에 sameParent 검증 추가 | 100% |
| 2026-07-18 | AI UX 개선 2차 (플래시 수정+임베딩 선별) | ①기존 대화방 진입 시 '무엇이든 물어보세요' 빈 상태가 로딩 동안 스치듯 보이던 플래시 제거 — AIChatLayout에 messagesLoading 상태+말풍선 스켈레톤(기존 방만, 새 방은 즉시 빈 상태), 방 빠른 전환 레이스 가드(selectedIdRef) ②"최근 20개" 개수 확대 대신 **임베딩 기반 선별**로 전환 — `lib/ai/select.ts` 신규: 최근 20개(최근성) + 전체 이력 임베딩 k-means(k≤15, 시간순 균등 초기화로 결정적, 6 iter, cosine)로 주제 클러스터별 중심 최근접 메모를 대표 선발, '(유사 메모 N개 대표)' 클러스터 크기를 빈도 가중치로 프롬프트 전달. 프롬프트는 35개 수준 유지하며 전체 이력 주제 커버(비용·lost-in-the-middle·최근성 희석 회피). 임베딩 <15개 또는 오류 시 recent-only 자동 강등(fail-open). insights 라우트 연결, gap/interest 프롬프트에 최근=지금 관심사/대표=지속 관심사 구분 지침, 응답·24h 캐시에 scope 메타 저장, 갭/관심사 UI 범위 문구 동적화('전체 메모 N개 기준 — 최근 20 + 주제별 대표 15'). 임베딩 풀 fetch 상한 2000개. 검증: tsc 0에러+ESLint 클린+합성 4클러스터(40/30/20/10) 정확 복원·결정성 단위 테스트 통과. 커밋 4c7d694, c645877 | 100% |
...
| 2026-08-02 | 백업 이미지 포맷 변환(WebP→JPG) + 즉시 실행 버튼 | 업로드 시 이미지가 WebP로 압축돼 R2에 저장되는데 백업이 그대로 Drive에 올려 **백업본이 전부 WebP** → 일반 뷰어에서 안 열리는 문제. `lib/backup/imageFormat.ts` 신규(`convertImageForBackup`/`backupImageKey`/`normalizeBackupImageFormat`) — **WebP만** JPG(q90) 변환, 알파 있으면 그 장만 PNG, jpg/png/gif/svg/mp4/pdf는 원본 유지(재인코딩 시 화질·애니메이션·벡터 손해), 변환 실패 시 원본으로 degrade. **포맷 선택 근거(실측 11장)**: JPG 전체 0.92배·평균 109ms vs PNG 3.46배·276ms — R2 원본이 이미 손실 압축이라 PNG 무손실 재인코딩은 화질 이득 0에 용량만 폭증(최대 5.1배 관측), 표본 알파 채널 0/10. 실데이터 구성도 확인: 본문 원본 149개 = jpg 81 · **webp 62** · png 6. **⚠️ 증분 판정을 파일명 전체→확장자 뗀 uuid로 수정** — 확장자가 바뀌면 기존 백업본과 이름이 달라져 매 주기 전량 재업로드될 뻔했음(첫 회만 변환 재업로드, 이후 정상 skip). `api/backup/images` 신규(즉시 실행, deadline 240s 후 `done:false`+`remaining` 반환 → 클라이언트가 완료까지 자동 재호출, 최대 20라운드) + 설정에 포맷 3버튼·'이미지 지금 백업' 버튼. `backupImageFormat` 설정 추가(기본 jpg). 기존 Drive의 webp 백업본은 **삭제하지 않고 그대로 둠**(사용자 결정 — r2-gc 오판 삭제 사고 이력상 백업 삭제 코드를 새로 만들지 않는 쪽이 안전, 중복 약 60MB는 감수). tsc 0에러+ESLint 클린+로컬 UI 렌더 확인 | 100% |
| 2026-08-02 | 🚨 잠금 메모 해제 불가 버그 수정 | **새로고침 후에는 올바른 비밀번호로도 잠금 해제가 영구 불가**했던 치명 버그. 원인: 해제는 암호문(`locked_content`)을 복호화하는데 **목록 쿼리 `LIST_COLS`에 그 컬럼이 없어**(성능 위해 본문 제외) `toMemo`가 `lockedContent: null`을 만들고, `MemoCard.handleUnlock`이 `memo.lockedContent!`(non-null 단언으로 타입 검사도 통과)를 그대로 넘겨 **복호화 함수가 null을 받아 실패** → UI엔 "비밀번호가 올바르지 않습니다"로 표시. 잠근 **직후**엔 `lockMemo`가 캐시에 `lockedContent: encrypted`를 심어둬 해제가 되지만, 새로고침하면 목록 재페치로 null이 덮여 그때부터 영구 실패(그래서 그동안 발견되지 않음). **데이터 유실은 없음** — 암호문은 DB에 온전. 수정: `unlockMemo(id, password)`로 시그니처 변경하고 **해제 시점에 해당 메모만 단건 조회**해 암호문 획득(목록은 계속 경량 유지, 암호문은 필요할 때만 전송). 호출부 3곳(MemoCard/MemoList/MemoListParts) 타입 동기화. 검증: 임시 메모 생성→잠금→**새로고침**(버그 재현 조건)→해제 성공, 마커 문장 원문 복원 확인, tsc 0에러+next build 통과 | 100% |
| 2026-08-02 | r2-gc 잔여 오판 경로 3종 차단 (예방) | 2026-07-05 사고 재발 방지 가드를 재점검하다 **가드 ③(memo_id 연결 파일 보존)이 사실상 무효**임을 발견 — `uploaded_files.memo_id`를 채우는 코드가 `restore/google-drive` 한 곳뿐이고 정상 업로드 경로(`/api/upload`)는 넣지 않아 **실데이터 18행 전부 memo_id=null**(발동 이력 0회). 잠금 메모는 content가 암호화라 URL 스캔이 불가한데 이를 대신할 판정 수단이 비어 있던 셈(현재 활성 잠금 메모 0개라 잠복 상태였음). 조치 3종: **④버전 이력 스캔 추가** — `memo_versions`도 URL 수집 대상에 포함(본문에서 지웠지만 이전 버전에만 남은 이미지를 orphan 오판 → 나중에 버전 복원해도 이미지가 이미 삭제된 상태가 되던 경로 차단). memo_id 50개씩 chunk + 200행 페이지네이션, 페치 실패 시 해당 사용자 GC 스킵(fail-safe) **⑤잠금 메모 fail-safe** — 활성 잠금 메모가 1개라도 있으면 memo_id 없는 파일은 "참조 없음"을 증명할 수 없으므로 무조건 보존 **⑥변형 URL도 참조로 인정** — 본문 image node의 src가 md_/thumb_ 변형인 경우가 있어 원본 URL만 비교하면 살아있는 이미지를 orphan 오판하고 삭제 시 변형까지 함께 지움(현재 데이터 0건이나 코드 경로는 열려 있었음). 부수: 파일마다 전체 본문을 `JSON.stringify`로 재순회하던 O(files×bodies) 판정을 **URL 집합 1회 구축**으로 교체(버전 1,047행까지 스캔해야 해서 비용 차이 큼). 검증: tsc 0에러+ESLint 클린, 프로덕션 DB 대상 dryRun `checked:15 deleted:0 kept:15 errorCount:0`(오판 삭제 0, 버전 1,047행 스캔이 에러 없이 완주해 ④ 경로 실행 확인). **⑤ 실전 검증 완료(같은 날 후속)**: 사용자가 메모 1개(`26.05.16_엔카…`)를 실제로 잠근 뒤 A/B 대조 — 잠금 상태 `keptBy:{lockFailsafe:15, referenced:0}` vs 잠금 없음 가정(코드로 재현) `keptBy:{lockFailsafe:0, referenced:15}`로 **판정 경로가 완전히 전환됨을 확인**(양쪽 다 deleted:0). 잠근 메모는 `content=null`·`content_text=''`·버전 이력 0개라 URL이 어디에서도 안 잡히는 상태 = 가드 ⑤가 없었으면 오판 삭제될 정확한 케이스였음. 함께 **보존 사유 계측 `keptBy{memoLink,lockFailsafe,referenced}`를 응답에 추가** — "가드는 있는데 발동한 적 없음"을 크론 로그만으로 조기 발견하기 위함(이번 문제의 재발 방지). 한계: ⑤ 발동 시(메모를 잠그기 시작하면) memo_id가 null인 기존 파일은 전부 보존되어 GC가 사실상 멈춤 — 스토리지 5.8MB/500MB라 감수 가능, 근본 해결은 업로드 시 memo_id 기록(후속 후보) | 100% |
| 2026-08-04 | 플래너 시간 드롭다운 선택이 롤백되던 버그 수정 (배포+프로덕션 검증 완료) | **TimePicker 드롭다운에서 시/분을 골라도 값이 원래대로 되돌아가던 버그.** 원인: 직접 입력 콤보로 개편(2026-07-30)하면서 `onFocus`가 현재 값을 대기 텍스트(`text`)로 채우게 됐는데, 옵션 클릭 핸들러가 `onCommit(선택값)` 직후 **`inputRef.current.blur()`를 직접 호출**했다. `blur()`는 `onBlur`→`commit(text)`를 **동기적으로** 실행하고, 그 핸들러 클로저의 `text`는 바로 앞줄 `setText(null)`이 아직 반영되지 않은 **직전 렌더의 옛 값**("09" 등)이라 방금 고른 값을 같은 배치에서 덮어썼다(마지막 `onChange`가 승리). 드롭다운은 포커스 상태에서만 열리므로 `text`가 항상 채워져 있어 시·분 모두 100% 재현. 수정: ①대기 텍스트를 `textRef`로 미러링(`setPending`)해 `commit()`이 항상 최신값을 읽도록 ②옵션 선택을 `mousedown`→`click`으로 옮기고 mousedown은 `preventDefault()`만 수행해 **포커스를 유지=blur 자체가 발생하지 않게**(`inputRef.blur()` 호출 제거) ③↑/↓ 증감도 `setPending(null)`을 커밋보다 먼저. **함께 처리한 2건**: ④**Escape가 모달을 통째로 닫던 문제** — `Modal`의 Escape 핸들러가 **capture 단계**라 드롭다운의 bubble 핸들러보다 먼저 실행돼 `stopPropagation`+닫기를 해버렸음(드롭다운만 닫으려다 플랜 폼이 사라짐). `Modal`이 `[data-escape-layer]` 요소 존재 시 Escape를 넘기도록 가드 추가 + TimePicker 패널에 표식 부여(다른 겹침 레이어도 이 속성만 달면 동일 보호) ⑤**CI Lint red 해소** — `WeekView.tsx`의 `lockScrollForRangeSelect`가 컴포넌트 본문 안에서 `document.body.style`을 수정해 `react-hooks/immutability` 에러(플러그인 업데이트로 신규 발생, 이번 변경과 무관하게 dev 전체 PR이 red였음). disable 주석 대신 **모듈 스코프 함수로 이동**(해제 함수를 반환 → `acquireRangeScrollLock`/`releaseRangeScrollLock` 래퍼가 ref 관리)해 규칙 대상에서 원천 제외. 검증: verify-changes.sh(null byte 0/파일 끝 정상/tsc 0에러)+ESLint 클린+next build 통과. **프로덕션 라이브 검증 통과(PR #328 머지 후, SW 캐시 삭제 후 테스트)**: 시 09→03 유지(소요 7시간 갱신)·분 00→45 유지(6시간 15분)·타이핑 "18"+Enter(14시간 15분)·↑2회 03→05·종료 24시 선택 시 24:00+분 필드 비활성·드롭다운 내부 스크롤 정상·콘솔 에러 0건 | 100% |
| 2026-08-07 | 4~7월 과거 플랜이 안 보이던 버그 배포 (프로덕션 검증 완료) | dev에만 있던 e9756cc(initialData→placeholderData)를 main 머지·배포(PR #329). **데이터 소실 아님** — Supabase 직접 조회로 4~7월 플랜 56개(완료 53개, 월별 4월 6·5월 13·6월 19·7월 18) 온전 확인. 원인은 LS 캐시를 모든 범위 쿼리 키의 initialData + 최신 ts로 넘겨 과거 달 범위가 staleTime(5분) 안에서 항상 신선 판정 → 서버 fetch 영구 생략(도입 7c96cfc, 2026-07-04). **프로덕션 라이브 검증 통과**: SW 해제+memo-planner-v5 캐시 삭제 후 월 뷰 4·5·6·7월 이동 시 완료 플랜 전부 표시(취소선 유지) | 100% |
| 2026-08-07 | 월 뷰 범위 플랜 침묵 누락 수정 | 7/23~26 나고야 여행(범위 플랜)이 월 뷰 19~25일 주에서 사라지고 26일 칸에만 보이던 문제. 원인: `getWeekRangePlans`가 `overlapping.slice(0, MAX_RANGE_BARS)`로 **겹치는 플랜 개수**를 먼저 잘랐음 — 해당 주는 4개가 걸치지만 슬롯 재사용(7/13~19와 7/23~26은 서로 안 겹침)으로 실제 필요한 줄은 2줄뿐이라, 자리가 남는데도 유입 순서상 4번째라는 이유만으로 통째 누락(정렬이 없어 어느 것이 잘릴지도 임의적). 수정: ①상한을 개수→**차지하는 줄 수**로 재정의(슬롯 할당 후 `slot >= MAX_RANGE_BARS`만 제외) ②greedy 슬롯 할당의 전제인 시작일 순 정렬 추가(동률은 id 안정 정렬) ③그래도 숨겨진 게 있으면 주 우측에 `+N` 배지(침묵 누락 방지) | 100% |
