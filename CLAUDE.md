...
- **vitest 도입**: `weave-local-tests`의 에디터 단위 테스트(heading boundary 18, isolateSelectedLines) 이관 + `npm test` + CI 연결. 현재 레포엔 테스트 러너가 없어 node로 직접 돌리는 절대경로 스크립트를 레포 밖에 보관 중(프로젝트 폴더 이동 시 경로 수정 필요). 별도 PR. (2026-09-24 논의)
