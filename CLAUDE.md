...
| 2026-07-05 | 그래프 검색 인덱스 복원 | 카운트 복원(전 항목) 배포 후 후속 — 복귀 시 검색어·총 개수는 살지만 인덱스가 3/11→1/11로 원복. 원인: search만 sessionStorage 저장, searchMatchIdx는 useState(0)이라 리마운트 리셋(복원 effect는 클램프만). 수정: searchMatchIdx도 sessionStorage(weave:graph-search-idx) 저장/복원, 기존 복원 effect 클램프가 범위 보장. 프로덕션 라이브 재현(3/11→복귀→1/11 확인) 후 수정. tsc 통과 | 100% |
