...
export function sourceNotePrompt(kind: SourceNotePromptKind): string {
  const inputGuide: Record<SourceNotePromptKind, string> = {
    ...
    images: [
      ...
      '- 인용 위치(loc)는 라벨의 번호를 그대로 써서 "이미지 2 · 조각 3"처럼 적으세요. 이미지 번호(N/M의 N)와 조각 번호를 혼동하지 마세요 — 긴 캡처 1장은 조각이 여러 개여도 모두 "이미지 1"입니다.',
    ].join('\n'),
    chunks: [
      ...
      '- 추출문 본문은 압축된 것이라 원문 문장이 아닙니다. **quotes는 맨 끝 [인용 후보] 목록에서만** 고르고, 문장을 다시 쓰지 말고 번호만 {"id": "Q3"} 형식으로 적으세요. 후보가 없으면 빈 배열.',
    ].join('\n'),
  }
  ...
  return [
    ...
    [
      ...
      '- 길이 상한: 섹션당 bullets 최대 6개, 불릿 하나는 2문장 이내. 넘치면 덜 중요한 것을 빼세요.',
      '- concepts 이름은 위키로 쓸 하나의 명사: 붙여쓰기, 괄호·가운뎃점(·) 없이 (예: "선행지표·후행지표" → "선행지표", "매출선형모델(S=…)" → "매출선형모델").',
    ...
    [
      ...
      '- 단, 뜻이 다른 기존 표기를 "비슷한 맥락"이라는 이유로 억지로 재사용하지 마세요 (예: 네이버 가이드에 "토스" 태그 X). 맞는 게 없으면 새로 만들거나 개수를 줄이세요.',
      '- 어휘 목록에 없는 새 위키는 최대 3개, source는 "new".',
      '- concepts 중 이 자료만의 핵심 개념은, 어휘 목록에 같은 뜻의 표기가 없으면 반드시 new 위키로 제안하세요 (기존 표기만으로 채우지 마세요).',
...
/** 분할 추출 출력에서 압축 본문과 원문 인용 후보를 가르는 표시 줄 */
export const QUOTE_MARKER = '===인용 후보==='

/**
 * 분할 추출(3-3) system — 이미지 속 내용을 구조 그대로 **압축 전사**한다.
 * 단, 인용 후보(따옴표·강조·결론 문장)는 압축하지 않고 QUOTE_MARKER 아래에 원문 그대로 따로 받는다.
 * 글자가 빽빽한 캡처를 글자 그대로 옮기면 출력 한도를 넘으므로(E2E 확인), 내용 단위는
 * 하나도 빼지 않되 문장은 짧게 줄인다. 종합 단계가 이 추출문만 보고 요약하므로 누락은 금지.
 */
export const SOURCE_CHUNK_EXTRACT_SYSTEM = [
  '당신은 긴 이미지 자료의 내용을 빠짐없이 옮기는 압축 전사 도우미입니다. 결과는 나중에 전체 요약의 유일한 재료가 됩니다.',
  '- 구조(제목·소제목·목록·표·캡션)를 마크다운으로 유지하세요.',
  '- 모든 소제목·주장·예시·수치·고유명사·인용할 만한 문장은 하나도 빼지 마세요.',
  '- 대신 문장은 짧게 줄이고(조사·수식어 생략 가능), 같은 문장이 반복되면 한 번만 적으세요. 본문은 1,200단어 이내.',
  ...
  '',
  '## 인용 후보 (압축 금지)',
  `본문 다음 줄에 정확히 "${QUOTE_MARKER}"라고 쓰고, 그 아래에 인용 후보를 한 줄에 하나씩 적으세요:`,
  '- "문장 원문" (이미지 2 · 조각 3)   ← PDF 페이지면 (p.3 · 조각 1)',
  '- 위치는 그 문장이 실제로 보이는 조각의 라벨 하나만 그대로 적으세요 (범위 "1~2" 금지). 소제목의 라벨을 물려 쓰지 말고, 문장 바로 위에 붙은 [라벨]을 보세요.',
  '- 대상 (빠짐없이):',
  '  · 배경색 하이라이트(형광펜)가 칠해진 문장, 굵은 글씨로 강조된 문장',
  '  · 인용 박스(왼쪽 세로선)·큰 따옴표로 묶인 문장 — 글 도입부의 강조 문장도 포함',
  '  · 단락·글 전체의 결론 문장',
  '- 이미지에 보이는 글자 그대로 옮기세요. 줄이거나 다듬거나 맞춤법을 고치지 마세요. 문장 일부만 자르지 마세요.',
  '- 구간당 최대 10개. 후보가 없으면 표시 줄과 목록을 생략하세요.',
  '- 본문 안에 인용문(> "…")을 넣었더라도 반드시 마지막 구획에 다시 적으세요 — 이 구획이 빠지면 노트의 "기억할 문장"이 비게 됩니다.',
  '- 본문 안 인용문(> "…")에도 끝에 같은 방식으로 조각 라벨 하나를 붙이세요: > "문장" (p.3 · 조각 1)',
  '- 본문과 인용 후보 외의 앞뒤 설명은 쓰지 마세요.',
].join('\n')
