...
import { Plus, LayoutGrid, List, Search, SortDesc, Trash2 } from 'lucide-react'
    ...
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-950">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
        ...
        <div className="flex rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          <button onClick={() => setView('card')} className={cn('p-1.5 transition-colors', view === 'card' ? 'bg-violet-100 dark:bg-violet-900/40 text-violet-600' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800')}>
            <LayoutGrid size={13} />
          </button>
          <button onClick={() => setView('list')} className={cn('p-1.5 transition-colors', view === 'list' ? 'bg-violet-100 dark:bg-violet-900/40 text-violet-600' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800')}>
            <List size={13} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {memos.length === 0 ? (
          ...
          <div className={view === 'card' ? 'p-4' : ''}>
            <MemoSection memos={filtered.all} view={view} isTrash {...cardActions} />
          </div>
        ) : (
          /* 일반 뷰 */
          <div className={view === 'card' ? 'p-4' : ''}>
